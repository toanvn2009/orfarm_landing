  ...
  class Radium_Theme_Importer {
    ...
    public function add_widget_to_sidebar( $sidebar_slug, $widget_slug, $count_mod, $widget_settings = array() ) {
      ...
      update_option( 'widget_'.$widget_slug, $newWidget );
		...
		do_action('wbc_after_update_widget_options');
    }
    ...
    public function set_demo_data( $file ) {
      ...
      if ( $importer_error ) {
        ...
        if ( !is_file( $file ) ) {
		  ...
		  do_action( 'wbc_importer_before_content_import', $this->demo_files_path );
    ...
    public function set_demo_theme_options( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Theme options Import file could not be found. Please try again.', LION_CURRENT_THEME ),
          '',
    ...
    function process_widget_import_file( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Widget Import file could not be found. Please try again.', LION_CURRENT_THEME ),
          '',
          array( 'back_link' => true )
        );
      }
    ...
    public function import_widgets( $data ) {
      ...
      foreach ( $data as $sidebar_id => $widgets ) {
        ...
        if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
          ...
          $sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', LION_CURRENT_THEME );
        }
        ...
        foreach ( $widgets as $widget_instance_id => $widget ) {
          ...
          if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
            ...
            $widget_message = __( 'Site does not support widget', LION_CURRENT_THEME ); // explain why widget not imported
          }
          ...
          if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
            ...
            foreach ( $single_widget_instances as $check_id => $check_widget ) {
              ...
              if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
                ...
                $widget_message = __( 'Widget already exists', LION_CURRENT_THEME ); // explain why widget not imported
          ...
          if ( ! $fail ) {
            ...
            if ( $sidebar_available ) {
              ...
              $widget_message = __( 'Imported', LION_CURRENT_THEME );
            } else {
              $widget_message_type = 'warning';
              $widget_message = __( 'Imported to Inactive', LION_CURRENT_THEME );
            }
          ...
          $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', LION_CURRENT_THEME ); // show "No Title" if widget instance is untitled
