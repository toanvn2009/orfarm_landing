...
class Lionthemes_Productscategory_Element extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'Products Category', 'orfarm' ); 
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'hide_categories',
			[
				'label' => __( 'Hide categories list', 'orfarm' ),
				'type' => \Elementor\Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'default' => 'no',
			]
	...
	protected function render() {
		...
		$_id = lionthemes_make_id();
		$loop = orfarm_woocommerce_query('',$settings['number'], $settings['category']);
		...
		$cat = get_term_by('slug', $settings['category'], 'product_cat'); 
		
		if ( $loop->have_posts() ){ 
		?>

			<?php $_total = $loop->found_posts > $settings['number'] ? $settings['number'] : $loop->found_posts; ?>
			<div class="woocommerce <?php echo esc_attr($settings['el_class']); ?>">
				<?php if ($settings['title'] || $settings['short_desc']) { ?>
		            <div id="countdown-event-<?php echo $_id ?>"  class="element-widget-title"> 	
						<?php if($settings['title']){ ?>
							<h3 class="vc_widget_title vc_products_title <?php echo (($settings['alignment'] != '') ? ' '. $settings['alignment'] : ''); ?>">
				...
				<div id="products-element-<?php echo $_id ?>" class="inner-content">
					...
					<?php wc_get_template( 'product-layout/'.$settings['style'].'.php', array( 
							...
							'hidecategories' => ($settings['hide_categories'] == 'true'),
							 ) ); ?>
