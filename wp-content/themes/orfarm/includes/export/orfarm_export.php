...
function orfarm_export_wp( $args = array() ) {
	global $wpdb, $post;
	 ...
	 */
	do_action( 'orfarm_export_wp', $args );
	...
	if ( $args['page_name'] && ( 'post' === $args['content'] || 'page' === $args['content'] ) ) {
		$where .= $wpdb->prepare( " AND {$wpdb->posts}.post_name = %s", $args['page_name'] );
	} 
	...
	if( $args['content'] == 'main') {
	     $post_ids = $wpdb->get_col( "SELECT ID FROM {$wpdb->posts}  WHERE {$wpdb->posts}.post_type IN ('post','attachment','custom_css','wp_block','wp_template','wp_template_part','wp_global_styles','e-landing-page','elementor_library','lionthemes_block','lionthemes_mega_menu','product','product_variation','wpcf7_contact_form','mc4wp-form') AND {$wpdb->posts}.post_status != 'auto-draft'" );
	} else {
		 $post_ids = $wpdb->get_col( "SELECT ID FROM {$wpdb->posts} $join WHERE $where" );
    }
	//echo "SELECT ID FROM {$wpdb->posts} $join WHERE $where"; die;
	//SELECT ID FROM wp_posts  WHERE wp_posts.post_type IN ('post','page','attachment','nav_menu_item','custom_css','wp_block','wp_template','wp_template_part','wp_global_styles','wp_navigation','e-landing-page','elementor_library','lionthemes_block','lionthemes_mega_menu','product','product_variation','shop_order','shop_order_refund','shop_coupon','wpcf7_contact_form','mc4wp-form') AND wp_posts.post_status != 'auto-draft'
	/*
	...
	if ( isset( $term ) && $term ) {
		...
		unset( $term, $cat );
	} elseif ( 'all' === $args['content'] || 'main1' === $args['content'] ) {
...
<channel>
	...
	<?php endforeach; ?>
	<?php
	if ( 'all' === $args['content'] || 'main1' === $args['content'] ) {
		wxr_nav_menu_terms();}
	?>
 ...
 */
 function orfarm_menu_exporter( $args = array() ) {
	global $wpdb, $post;
