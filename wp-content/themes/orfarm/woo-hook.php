...
function orfarm_shop_toolbar_view_mode() { 
	...
	<div class="view-mode">
		...
		?>
		          <a data-toggle="tooltip" href="<?php echo $link ;?>" data-mode="<?php echo $key; ?>" class="<?php if( isset( $_GET['view_mode'] ) && $_GET['view_mode']== $key || $_GET['column'] == $key){ echo 'active';} ?> <?php if( $orfarm_viewmode == $key && !isset($_GET['view_mode'])) { echo 'is_default active'; } ?>" title="<?php echo $viewMode['name']; ?>"><i class="<?php echo $viewMode['icon'];?>"></i></a>
		<?php } ?>
	</div>
<?php }
...
function orfarm_woocommerce_breadcrumbs() {
	 $orfarm_opt = get_option( 'orfarm_opt' );
	 if ($orfarm_opt['page_breadcrumb'] == 0) return array();
    return array(
            'delimiter'   => '',
...
function orfarm_cart_side_content()
{
	...
	if (!empty($orfarm_opt['sticky_icons'])) {
		...
		if (count($sticky_icons)) {
			...
			if (!empty($sticky_icons['cart']) && class_exists('WooCommerce') && !is_cart() && !is_checkout()) echo '<li class="quick-cart"><a class="toggle-cartside" href="javascript:void(0)"><span class="badge">' . WC()->cart->get_cart_contents_count() . '</span><span class="icon-cart icons"></span></a></li>';
